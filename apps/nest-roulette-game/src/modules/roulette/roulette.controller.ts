...
import { DeleteRequest } from '../../models/dto/request/delete.request';
import { SpinRequest } from '../../models/dto/request/spin/spin.request';
import { CreateResponse } from '../../models/dto/response/create.request';
import { DeleteResponse } from '../../models/dto/response/delete.request';
import { SpinResponse } from '../../models/dto/response/spin.request';

@Controller()
export class RouletteController {
  constructor(private readonly jwtService: JwtService, private readonly rouletteService: RouletteService) {}

  @Post('create')
  public async create(@Req() request: Request, @Body() createRequest: CreateRequest): Promise<CreateResponse> {
    ...
    // initialize session
    const resp = await this.rouletteService.initializeSession(balance, createRequest.userId, createRequest.gameMode);
  ...
  public async spin(@Body() spinRequest: SpinRequest): Promise<SpinResponse> {
    ...
    const updatedGameSession = await this.rouletteService.updateBalance(spinRequest, amount);

    return { message: 'updated balance: ' + updatedGameSession.balance };
  }

  @Delete('end')
  public async end(@Body() deleteRequest: DeleteRequest): Promise<DeleteResponse> {
    const userGameSession = await this.rouletteService.getUserGameSession(deleteRequest.userId);
